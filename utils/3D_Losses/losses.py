...
SUPPORTED_LOSSES = ['ce', 'bce', 'wce', 'pce', 'dice', 'gdl', 'dice_msqe', 'dice_ce']
...
class DiceCoefficient:
    ...
    def __call__(self, input, target, is_training):
        # Average across channels in order to get the final score

        # assert len(input) == len(target)
        #
        # long_target = target.long()
        # long_target_ = torch.unsqueeze(long_target, 1)
        #
        # long_input = input.long()
        #
        # var_target = Variable(long_target_)
        # shape = list(var_target.size())
        #
        # var_input = Variable(long_input)
        # shape_input = list(var_input.size())
        #
        # y_onehot = torch.cuda.FloatTensor(shape[0], shape_input[1], shape[2], shape[3], shape[4]).zero_()
        #
        # y_onehot.scatter_(1, torch.cuda.LongTensor(long_target_), 1)
        #
        # # eps = 1e-5
        # y_input = F.softmax(input, dim=1)
        # dice_score = 0
        #
        # for i in range(shape[0]):
        #   intersection = torch.sum(y_input[i] * y_onehot[i])
        #   union = self.epsilon + torch.sum(y_input[i]) + torch.sum(y_onehot[i])
        #   dice_score_img = (2 * intersection / (union))
        #   dice_score += dice_score_img
        #
        # dice_score /= shape[0]
        if is_training:
            ch_dice = compute_per_channel_dice(input, target, epsilon=self.epsilon, ignore_index=self.ignore_index)

            return torch.mean(ch_dice)
        else:
            value, index = torch.max(input, 1)
            input = expand_as_one_hot(index, C=input.size()[1], ignore_index=self.ignore_index)

            ch_dice = compute_per_channel_dice(input, target, epsilon=self.epsilon, ignore_index=self.ignore_index)
            return ch_dice
        # print(dice_score - torch.sum(ch_dice) / torch.sum(ch_dice > 0))

        # return   # torch.mean(ch_dice[torch.unique(target).long()])
        # torch.sum(ch_dice) / torch.sum(ch_dice > 0)  # dice_score.cpu().detach().numpy()
...
class MSQE_Loss(nn.Module):
    """
    """

    def __init__(self, device):
        super(MSQE_Loss, self).__init__()
    ...
    def forward(self):
        ...
        for i in range(self.weights.__len__()):
            loss.add_(F.mse_loss(self.quantized_weights[i], self.weights[i]))
        return loss * 0.05

    def set_weights(self, weights, quantized_weights):
        self.quantized_weights = quantized_weights
        self.weights = weights


class Dice_MSQE_Loss(nn.Module):
    """
    """

    def __init__(self, weight=None, ignore_index=None, device='cpu'):
        super(Dice_MSQE_Loss, self).__init__()
        self.dice_loss = DiceLoss(weight=weight, ignore_index=ignore_index)
        self.msqe_loss = MSQE_Loss(device=device)

    def forward(self, input, target):
        return torch.add(self.dice_loss(input,target), self.msqe_loss())
...
class Dice_CrossEntropy_Loss(nn.Module):
    ...
    def __init__(self, all_labels, weight=None, ignore_index=None, device='cpu'):
        super(Dice_CrossEntropy_Loss, self).__init__()
        self.dice_loss = DiceLoss(weight=weight, ignore_index=ignore_index)
        self.ce_loss = CrossEntropyLossForDice(all_labels, device)

    def forward(self, input, target):
        return torch.add(0.5 * self.dice_loss(input, target), 0.5 * self.ce_loss(input, target))


class CrossEntropyLossForDice(nn.Module):
    """
    """

    def __init__(self, all_labels, device, weight=None, ignore_index=-1):
        super(CrossEntropyLossForDice, self).__init__()
        self.register_buffer('weight', weight)
        self.ignore_index = ignore_index
        self.class_weights = self._class_weights(all_labels)
        if self.weight is not None:
            weight = Variable(self.weight, requires_grad=False)
            self.class_weights = self.class_weights * weight
        self.loss = nn.CrossEntropyLoss(weight=torch.from_numpy(self.class_weights).type(torch.FloatTensor).to(device),
                                        ignore_index=self.ignore_index)

    def forward(self, input, target):
        return self.loss(input, target.long())
    ...
    def _class_weights(labels):
        ...
        class_freq = np.zeros((1, num_classes))
        for i in range(num_classes):
            class_freq[0, i] = np.sum(labels == i) / len(labels)
        median_freq = np.median(class_freq)
        for i in range(num_classes):
            class_weights[i] = median_freq / class_freq[0, i]
        return class_weights
...
def expand_as_one_hot(input, C, ignore_index=None):
    ...
    if ignore_index is not None:
        ...
        # scatter to get the one-hot tensor
        return torch.zeros(shape).to(input.device).scatter(1, src.type(torch.cuda.LongTensor), 1)
        # return torch.zeros(shape).to(input.device).scatter(1, src, 1)


def get_loss_criterion(loss_str, weight=None, ignore_index=None, device='cpu', all_labels=None):
    """
    ...
    elif loss_str == 'dice_msqe':
        return Dice_MSQE_Loss(weight=weight, ignore_index=ignore_index, device=device)
    elif loss_str == 'dice_ce':
        assert all_labels is not None, f'{loss_str} was chosen but the all_labels argument is None, should be valued'
        return Dice_CrossEntropy_Loss(all_labels, weight=weight, ignore_index=ignore_index, device=device)
